---
id: ew-easyway-webapp-02-logiche-easyway-index
title: logiche-easyway-index
summary: Indice delle logiche EasyWay (login/onboarding, audit/logging, notifiche, API esterne, policy).
status: active
owner: team-platform
tags: [domain/frontend, layer/index, audience/dev, privacy/internal, language/it, docs]
llm:
  include: true
  pii: none
  chunk_hint: 250-400
  redaction: [email, phone]
entities: []
updated: '2026-01-05'
next: Aggiornare quando si aggiungono nuove pagine o capitoli.
---

[Home](../../../../docs/project-root/DEVELOPER_START_HERE.md) > [[domains/frontend|frontend]] > [[Layer - Index|Index]]

# Indice

- Nota per autori: per nuove pagine usare `./_template.md`.


Breadcrumb: Home / EasyWay WebApp / Logiche EasyWay

- [api-esterne-integrazione.md](./api-esterne-integrazione.md) - Integrazione API esterne
- [best-practice-naming-and-scalability.md](./best-practice-naming-and-scalability.md) - Best practice naming e scalabilita
- [logging-and-audit.md](./logging-and-audit.md) - Logging e audit
- [login-flussi-onboarding.md](./login-flussi-onboarding.md) - Flussi login e onboarding
- [notifiche-gestione.md](./notifiche-gestione.md) - Gestione notifiche
- [policy-di-configurazione-and-sicurezza-microservizi-e-api-gateway.md](./policy-di-configurazione-and-sicurezza-microservizi-e-api-gateway.md) - Policy di configurazione e sicurezza (microservizi e API gateway)
- [raccomandazione-architetturale-easyway-data-portal.md](./raccomandazione-architetturale-easyway-data-portal.md) - Raccomandazione architetturale

## Domande a cui risponde
- Dove trovo le guide per login/onboarding e notifiche?
- Dove sono le regole di logging/audit e le policy di sicurezza?
- Quali pagine sono canoniche per integrazioni con API esterne?



