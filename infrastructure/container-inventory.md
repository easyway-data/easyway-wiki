...
| `easyway-runner` | `agent-runner` | `easyway-prod-agent-runner` | Agent Task Runner v2.0 — 40 agenti, HTTP task server :8400, esecuzione server-side | 8400 | easyway-net | ~3.8 GB. Monta agents + wiki + scripts. Health: `GET /health`. Tasks: levi-scan, gedi-validate, wiki-ingest, health-report, exec |
...
| `easyway-storage-s3` | `minio` | `minio:RELEASE.2024-01-31` | Sovereign Object Storage S3 — documenti, report, backup | 9000, 9001 | easyway-net | Console :9001. Healthy (S96). Bucket: documents, reports, backups. Init container auto-crea bucket al boot |
